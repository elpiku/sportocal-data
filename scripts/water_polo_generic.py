#!/usr/bin/env python3
"""
Targeted Water Polo Scraper for sportocal-data.
Fetches NCAA Men's and Women's Water Polo tournaments.
Outputs to water-polo/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "water-polo" / "espn-all"
...
def main():
    ...
    print(f"Scraping Water Polo competitions ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"waterpolo-{slug}",
                sport_key=f"water-polo-{slug}",
                date_from=date_from,
                date_to=date_to,
                round_prefix="Match",
            )
