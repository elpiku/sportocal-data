#!/usr/bin/env python3
"""
Targeted Boxing Scraper for sportocal-data.
Fetches Top Rank, Matchroom, PBC, and major World Championship boxing events.
Outputs to boxing/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "boxing" / "espn-all"
...
def main():
    ...
    print(f"Scraping Boxing events ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"boxing-{slug}",
                sport_key=f"boxing-{slug}",
                date_from=date_from,
                date_to=date_to,
                round_prefix="Fight Card",
            )
