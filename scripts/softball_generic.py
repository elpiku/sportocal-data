#!/usr/bin/env python3
"""
Targeted College Softball Scraper for sportocal-data.
Fetches NCAA Women's College Softball & Women's College World Series (WCWS).
Outputs to softball/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "softball" / "espn-all"
MIN_EVENTS_THRESHOLD = 3

TARGET_LEAGUES = {
    "college-softball": "NCAA Women's College Softball",
}
...
def main():
    ...
    print(f"Scraping Softball competitions ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"softball-{slug}",
                sport_key=f"softball-{slug}",
                date_from=date_from,
                date_to=date_to,
                round_prefix="Game",
            )
