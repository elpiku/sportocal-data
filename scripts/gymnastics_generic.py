#!/usr/bin/env python3
"""
Targeted Gymnastics Scraper for sportocal-data.
Fetches NCAA Women's College Gymnastics events and meets.
Outputs to gymnastics/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "gymnastics" / "espn-all"
MIN_EVENTS_THRESHOLD = 3

TARGET_LEAGUES = {
    "womens-college-gymnastics": "NCAA Women's Gymnastics",
}
...
def main():
    ...
    print(f"Scraping Gymnastics meets ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"gymnastics-{slug}",
                sport_key=f"gymnastics-{slug}",
                date_from=date_from,
                date_to=date_to,
                round_prefix="Meet",
            )
