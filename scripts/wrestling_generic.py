#!/usr/bin/env python3
"""
Targeted Wrestling Scraper for sportocal-data.
Fetches NCAA Division I College Wrestling matches and championships.
Outputs to wrestling/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "wrestling" / "espn-all"
MIN_EVENTS_THRESHOLD = 3

TARGET_LEAGUES = {
    "college-wrestling": "NCAA College Wrestling",
}
...
def main():
    ...
    print(f"Scraping Wrestling competitions ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"wrestling-{slug}",
                sport_key=f"wrestling-{slug}",
                date_from=date_from,
                date_to=date_to,
                round_prefix="Dual/Championship",
            )
