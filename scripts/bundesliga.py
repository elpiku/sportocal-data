#!/usr/bin/env python3
"""
Bundesliga (Germany) schedule scraper for the sportocal-data repo.
See espn_soccer_common.py for the shared ESPN Core API fetching logic
and its caveats.

Run this from the repo root: python scripts/bundesliga.py
"""
...
OUTPUT_PATH = Path(__file__).resolve().parent.parent / "football" / "bundesliga" / f"{SEASON_START_YEAR + 1}.json"

if __name__ == "__main__":
    run_league_scraper(
        league_slug="ger.1",
        id_prefix="bundesliga",
        ...
        min_events=80,  # a full 34-matchweek Bundesliga season has 306 games
    )
