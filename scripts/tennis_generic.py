#!/usr/bin/env python3
"""
Targeted Tennis Scraper for sportocal-data.
Fetches ATP Tour, WTA Tour, and Grand Slam tournament schedules from ESPN Core API.
Outputs to tennis/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "tennis" / "espn-all"
...
def main():
    ...
    print(f"Scraping {len(TARGET_LEAGUES)} targeted tennis tours ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"tennis-{slug}",
                sport_key=f"tennis-{slug}",
                date_from=date_from,
                date_to=date_to,
