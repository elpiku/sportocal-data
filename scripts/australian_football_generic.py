#!/usr/bin/env python3
"""
Targeted Australian Rules Football Scraper for sportocal-data.
Fetches AFL and AFLW competitions.
Outputs to australian-football/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "australian-football" / "espn-all"
MIN_EVENTS_THRESHOLD = 3

TARGET_LEAGUES = {
    "afl": "AFL (Australian Football League)",
    "aflw": "AFLW (AFL Women's)",
}
...
def main():
    ...
    print(f"Scraping {len(TARGET_LEAGUES)} targeted AFL competitions ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"afl-{slug}",
                sport_key=f"afl-{slug}",
                date_from=date_from,
                date_to=date_to,
                round_prefix="Round",
            )
