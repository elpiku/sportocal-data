#!/usr/bin/env python3
"""
Targeted Action Sports Scraper for sportocal-data.
Fetches Summer and Winter X Games events.
Outputs to action-sports/espn-all/<slug>.json.
"""
...
PAST_DAYS = 14
FUTURE_DAYS = 270
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "action-sports" / "espn-all"
...
def main():
    ...
    print(f"Scraping Action Sports events ({date_from} to {date_to})...", file=sys.stderr)
    written, skipped = 0, 0
    ...
    for slug, preferred_name in TARGET_LEAGUES.items():
        ...
        try:
            events = build_events(
                sport=SPORT,
                league_slug=slug,
                id_prefix=f"action-{slug}",
                sport_key=f"action-sports-{slug}",
                date_from=date_from,
                date_to=date_to,
                round_prefix="Event",
            )
